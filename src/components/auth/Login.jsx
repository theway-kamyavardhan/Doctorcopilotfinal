...
function MobileLogin({
  ...
  return (
    <div className={`flex flex-col min-h-[100svh] w-full font-sans antialiased ${isDark ? 'bg-black text-white' : 'bg-[#F2F2F7] text-black'}`}>
      <main className="flex-1 px-6 pt-16 pb-8 overflow-y-auto">
        ...
        {/* Role Selector */}
        <div className={`flex p-1 mb-8 rounded-xl ${isDark ? 'bg-[#1C1C1E]' : 'bg-[#E5E5EA]'}`}>
          {ROLES.map((r) => (
            <button
              key={r.id}
              ...
              className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-colors ${
                role === r.id
                  ? (isDark ? 'bg-[#3A3A3C] text-white shadow-sm' : 'bg-white text-black shadow-sm')
        ...
        {loginError && (
          <div className="mb-6 p-4 rounded-xl bg-red-100 text-red-700 text-sm font-semibold">
            {loginError}
          </div>
        )}
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div className={`rounded-xl overflow-hidden ${isDark ? 'bg-[#1C1C1E]' : 'bg-white'}`}>
            <div className={`px-4 py-2 border-b ${isDark ? 'border-[#38383A]' : 'border-[#E5E5EA]'}`}>
              <label className={`text-[11px] font-semibold uppercase tracking-wider ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                Identifier
              </label>
              <input
                type="text"
                ...
                className="w-full bg-transparent border-none outline-none text-[17px] py-1"
          ...
          <button
            ...
            style={{ touchAction: 'manipulation' }}
            className="w-full min-h-[50px] rounded-xl bg-[#007AFF] text-white text-[17px] font-bold flex items-center justify-center active:scale-[0.98] transition-transform disabled:opacity-50 mt-6"
          >
        ...
        {demoAccountsForRole.length > 0 && (
          <div className="mt-12">
            <h3 className={`text-[13px] font-semibold uppercase tracking-wider mb-3 px-2 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
              Demo Accounts
            </h3>
            <div className={`rounded-xl overflow-hidden ${isDark ? 'bg-[#1C1C1E]' : 'bg-white'}`}>
              {demoAccountsForRole.map((acc, idx) => (
                <button
                  key={acc.id}
                  onClick={() => applyDemoAccount(acc)}
                  style={{ touchAction: 'manipulation' }}
                  className={`w-full text-left px-4 py-3 flex justify-between items-center ${
                    idx !== demoAccountsForRole.length - 1 ? (isDark ? 'border-b border-[#38383A]' : 'border-b border-[#E5E5EA]') : ''
                  }`}
                >
                  <span className="text-[17px]">{acc.label}</span>
                  <span className={`text-[15px] ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{acc.identifier}</span>
                </button>
              ))}
            </div>
          </div>
