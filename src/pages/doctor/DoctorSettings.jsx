...
function Field({ label, value, onChange, isDark, textarea = false, disabled = false, placeholder = "" }) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className={`mt-2 text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>
          Update your visible profile, clinic context, and consultation details used across the doctor workspace.
        </p>
      </div>
        ...
        <section className={`rounded-[1.8rem] border p-6 ${isDark ? "border-white/10 bg-white/[0.03]" : "border-slate-200 bg-slate-50/80"}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className={`text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                Read-only credentials tied to your licensed doctor account.
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className={`rounded-2xl px-4 py-4 text-sm leading-7 ${isDark ? "bg-white/5 text-slate-300" : "bg-white text-slate-600"}`}>
              Password and security are still managed through the secure backend authentication flow. This page focuses on the professional profile shown to patients and other doctors.
            </div>
          </div>
        </section>
      </form>
